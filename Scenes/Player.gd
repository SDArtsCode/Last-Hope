...
func get_input():
	...
	match Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["type"]:
		"basic_gun": #{"type": "basic_gun", "damage": 5, "shoot_speed": 1.0, "shots_per_click": 4, "spacing": 0.1} (spacing is time between each bullet in a click)
			...
			if Input.is_action_just_pressed("shoot") and timer >= Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["shoot_speed"]:
				timer = 0.0
				
#					if Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["spacing"] < 0.05:
				if Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["spacing"] >= 0.1:
					$Weapon.bullet_per_shot = 1
					for i in range(Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["shots_per_click"]):
						$Weapon.fire()
						yield(get_tree().create_timer(Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["spacing"]), "timeout")
				
				else:
					$Weapon.bullet_per_shot = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["shots_per_click"]
					$Weapon.fire()
			
			$AimPosition.position = (get_local_mouse_position()).normalized() * 64
			$Weapon.accuracy = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["accuracy"]
			$Weapon.damage = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["damage"]
		...
		"basic_melee": # {"stats": {"type": "basic_melee", "damage": 10, "swing_speed": 0.4, "length": 0.75}
			$Melee.melee_damage = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["damage"]
			$Melee.melee_distance = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["length"] * 100
			$Melee.melee_speed = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["swing_speed"]
		...
		"close_melee": # {"type": "close_melee", "dps": 60, "length": 0.5} (think chainsaw, constant dps at a short tange)
			...
			$Laser.laser_distance = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["length"]*100
			$Laser.laser_damage = Global.blueprints[Global.player_parts["primary_arm"]]["stats"]["dps"]/5
...
func _input(event):
	if event.is_action_pressed("switch"):
		var temp = Global.player_parts["primary_arm"]
		Global.player_parts["primary_arm"] = Global.player_parts["secondary_arm"]
		Global.player_parts["secondary_arm"] = temp
